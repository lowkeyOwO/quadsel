...
if (isset($_POST['submit'])) {
    ...
    if (trim($name) !== '' && trim($email) !== '' && trim($phone_number) !== '' && strlen(trim($phone_number)) === 10 && trim($address) !== '' && trim($pan_no) !== '' && strlen(trim($pan_no)) === 10 && trim($aadhar_no) !== "" && strlen(trim($aadhar_no)) === 12 && ($dob !== '' || $dob !== null) && ($date_of_join !== '' && $date_of_join !== null) && $profile_photo_link !== '' && $password !== '' && $Confirmpassword !== '' && $Confirmpassword === $password) {
        $sql = "INSERT INTO pending_employee_details(emp_name,date_of_birth,address,email_id,aadhar_no,pan_no,password,profile_photo_link,phone_number,data_of_joining) VALUES('$name','$dob','$address','$email','$aadhar_no','$pan_no','$password','$profile_photo_link','$phone_number','$date_of_join')";
        ...
        <div class="SignUp-Form-Img">
            ...
            <form class='Employee-SignUp-Form' action="" method='post'>
                ...
                <div class="Employee-SignUp-Submitbtn">
                    ...
                    <p>Already have an account? <a href="/Form/Form.php">Login</a></p>
                </div>
        </div>
        <input type="hidden" name="profile_photo_link" id='profile_photo_link' value='<?php echo $profile_photo_link ?>'>
    </div>
    </form>
    </div>
    </div>
